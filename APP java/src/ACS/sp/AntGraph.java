...
public class AntGraph implements Serializable
{
    ...
    public AntGraph(int nNodes, double[][] delta, double[][] tau)
    {
        if(delta.length != nNodes)
            JOptionPane.showMessageDialog(null,"The number of nodes doesn't match with the dimension of delta matrix");
    ...
    private double average(double matrix[][])
    {
        ...
        for(int r = 0; r < m_nNodes; r++)
        {
            ...
            {
                if(matrix[r][s] != 0) // Adicionei aqui para evitar que ele use os n�s desnecess�rios.
                dSum += matrix[r][s];
            }
